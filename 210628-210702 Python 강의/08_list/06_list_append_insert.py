...
# 사용자에게 5개 값을 입력받아서 리스트에 저장하는 코드
# scores = []
...
# insert 함수를 이용해서 맨뒤에 삽입 - append를 활용하는 것이 일반적
nums.insert(len(nums),12.3)
print(nums)

nums.insert(len(nums)-1,[10, 20]) # 삽입되는 위치는 ? 마지막 원소 앞에 하위 리스트 삽
